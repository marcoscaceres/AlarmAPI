...
define(function(require) {
	 ...
	 * 
	 *  window.taskSchedule.add(new Date(), true).then(function(){ alert("OK")}, function(){ alert("FAIL")});   <-- with Promise
	 *  window.taskSchedule.add(new Date(), true) <-- without Promise
	 * 
	 */
	 ...
	 * 
	 * Make a request to the system to retrieve the tasks that were registered by the current application 
	 ...
	 * 
	 *  window.taskScheduler.getPendingTasks().then(function(tasks) {
	 	alert("There are " + tasks.length + " tasks set.");}, function(){ alert("FAIL")});  
	 * 
	 */
	...
	TaskScheduler.prototype.getPendingTasks = function() {
		return new Promise(function(resolver) {
			...
			for (var i = 1; i <= localStorage.length; i++){
				console.log("item ("+i+ ") with key "+ localStorage.key(i)
				...
				//display full item
				console.log("stored item date: "+ tmp_stored_item['date']);
				...
				if ( current_date - (new Date (tmp_stored_item['date'])) <= 0 ){
					...
					//remove old task
					console.log ("Old task will BE REMOVED, date: " + tmp_stored_item['date'] );
